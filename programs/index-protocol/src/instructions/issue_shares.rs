...
use anchor_spl::token::{self, Mint, MintTo, Token, TokenAccount, TransferChecked as SplTransferChecked};
...
pub fn handler<'info>(
    ...
    let user_authority_info = ctx.accounts.user.to_account_info();
    ...
    let user_shares = quantity
        .checked_sub(fee_shares)
        .ok_or(IndexError::NetQuantityZeroAfterFees)?;
    require!(user_shares > 0, IndexError::NetQuantityZeroAfterFees);
    ...
    for (i, asset) in assets.iter().enumerate() {
        ...
        let user_account_info = ctx.remaining_accounts[(i * 3) + 1].clone();
        let vault_account_info = ctx.remaining_accounts[(i * 3) + 2].clone();
    ...
    let cpi_accounts = MintTo {
        ...
        authority: ctx.accounts.index_config.to_account_info(),
    };
    ...
    token::mint_to(cpi_ctx, user_shares)?;
    ...
    if fee_shares > 0 {
        ...
        token::mint_to(fee_cpi_ctx, fee_shares)?;
    ...
    msg!(
        "Issued {} shares to user and {} shares as fees.",
        user_shares,
        credited_fee_shares
    );
    Ok(())
